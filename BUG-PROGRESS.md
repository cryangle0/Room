...
> Store：`rr_biz_v5`  
> 更新：2026-08-01（现网皮肤本地化 W0–W5 + Excel 回归）

## 总览（相对 Excel）

| 结论 | 说明 |
|------|------|
| **Excel 条目均已改** | #1–#14 均有对应实现，本轮逐项 DOM/Store 复测通过 |
| **MCP 门禁** | Excel **22/22**；皮肤壳 `_skin_gate.js` **17/17**（平台/品牌/买手/小程序） |
| **现网皮肤** | `assets/vendor/ontime/` 本地托管主题 CSS + iconfont；`index.html` 不再热链 |
| **非本表** | 真实现网/金蝶 HTTP；商品实拍图；100% 像素叠图 |
...
| #9 | 平台 | 订单/补货独立卡片；无订单类型字段 | 已改（备注文案去「订单类型」字样） | pass |
...
| 2026-08-01 | 本地买手 C 端全量 | **21/21 pass** | 首页/补货/选款/订单/个人/消息；5 分类全点；**16/16 品牌**商品列表；季度/编码视图/侧栏/品牌介绍·LOOKBOOK/商品详情/选款编辑/订单三态+详情/个人三栏/顶栏 active |
| 2026-08-01 | 本地 Excel 逐项 | **22/22 pass** | 对照 xlsx #1–#14；修正门禁导航（品牌行/顶栏再进子页）；编码视图 `data-view=code` 实测可切；#7 hideAll→buyerGoods 不可见 |
| 2026-08-01 | 现网皮肤 W0–W5 | **17/17 skin + 22/22 excel** | vendor 23 CSS；无热链 iconfont；顶栏黑/60px；主色紫；买手 iconfont；分支 `feat/ontime-skin-parity` |
| 2026-07-31 | Pages | 需硬刷新 | CDN/浏览器缓存可能导致短暂旧包；以本地门禁为准，推送后请 `Ctrl+F5` |

### 未纳入「通过」的项（诚实边界）

- 与现网截图像素级一致：未做  
- **真实**现网 / 金蝶 HTTP：未接（金蝶为本地模拟页；LOOK/添加品牌/详情入选款/删款/申请权限/登出/实时下钻已本地闭环）  
...
| （本轮） | 买手全量门禁 21/21：补商品详情/订单详情/消息页；修空季列表；全品牌回归 |
